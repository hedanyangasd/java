...
public class SetMenu {
	...
	public void register(){
		...
		while(!username.matches("^[a-zA-Z][a-zA-Z0-9_]{5,19}$")){
		...
		while(showname.length()<3 || showname.length()>20){
			System.out.println("输入不对，请重新输入！");
			showname = input.nextLine();
		}
		
		//输入密码以及确认密码
		System.out.println("请输入密码：(密码必须包含字母数字和特殊符号，密码最短为 8 位，最长不能超过 30)");
		String password = input.nextLine();
		while(!password.matches("^(?![A-Za-z0-9]+$)(?![A-Za-z\\W]+$)[a-zA-Z0-9\\W]{8,30}$")){
			//排除只有字母或数字与特殊符号和只有字母和数字和只有字母和特殊符号和只有字母和数字的组合
			System.out.println("输入不对，请重新输入！");
		...
		//输入邮箱
		System.out.println("请输入邮箱：(符合邮箱格式，最常不能超过 50 个字符)");
		String email=input.nextLine();
		while(!email.matches("[0-9a-zA-Z]{1,}@[0-9a-zA-Z]{1,}\\.(com|cn)")){
			System.out.println("输入不对，请重新输入！");
			...
			if(success){
				...
				for(User user:users){
					if(user.getUsername().equals(username)&&user.getAnswer().equals(answer)&&user.getQuestion()==question){
						System.out.println("答案和用户注册时填写的一致");
						System.out.println("请输入密码：(密码必须包含字母数字和特殊符号，密码最短为 8 位，最长不能超过 30)");
						String password = input.nextLine();
						while(!password.matches("^(?![A-Za-z0-9]+$)(?![A-Za-z\\W]+$)[a-zA-Z0-9\\W]{8,30}$")){
							//排除只有字母或数字与特殊符号和只有字母和数字和只有字母和特殊符号和只有字母和数字的组合
						...
						System.out.println("用户"+user.getUsername()+"的新密码为"+user.getPassword());
						pipei=true;
						break;
					}
				}
				if(!pipei){
					System.out.println("密码错误，不能修改密码");
				}
